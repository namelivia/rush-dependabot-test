...
function removeLinks(targetRootFolder, deployMetadataObject) {
...
function createLinks(targetRootFolder, deployMetadataObject) {
    for (const linkInfo of deployMetadataObject.links) {
        // Link to the relative path for symlinks
        const newLinkPath = path.join(targetRootFolder, linkInfo.linkPath);
...
function main() {
    ...
    const deployMetadataPath = path.join(targetRootFolder, 'deploy-metadata.json');
    if (!fs.existsSync(deployMetadataPath)) {
        throw new Error('Input file not found: ' + deployMetadataPath);
    }
    const deployMetadataJson = fs.readFileSync(deployMetadataPath).toString();
    const deployMetadataObject = JSON.parse(deployMetadataJson);
    if (args[0] === 'create') {

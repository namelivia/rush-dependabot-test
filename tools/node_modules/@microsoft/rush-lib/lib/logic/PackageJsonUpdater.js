...
class PackageJsonUpdater {
     ...
     */
    async doRushAddAsync(options) {
        const { projects, packagesToAdd, devDependency, updateOtherPackages, skipUpdate, debugInstall, variant } = options;
        const { DependencyAnalyzer } = await Promise.resolve().then(() => __importStar(require('./DependencyAnalyzer')));
    ...
    async _getNormalizedVersionSpec(projects, packageName, initialSpec, implicitlyPreferredVersion, explicitlyPreferredVersion, rangeStyle) {
        ...
        if (initialSpec) {
            ...
            if (initialSpec === explicitlyPreferredVersion) {
                console.log(safe_1.default.green('Assigning "') +
                    safe_1.default.cyan(initialSpec) +
                    safe_1.default.green(`" for "${packageName}" because it is the preferred version listed in ${RushConstants_1.RushConstants.commonVersionsFilename}.`));
                return initialSpec;
            }
        }
        if (this._rushConfiguration.ensureConsistentVersions && !initialSpec) {
            if (implicitlyPreferredVersion) {
                console.log(`Assigning the version "${safe_1.default.cyan(implicitlyPreferredVersion)}" for "${packageName}" ` +
                    'because it is already used by other projects in this repo.');
                return implicitlyPreferredVersion;
            }
            if (explicitlyPreferredVersion) {
                console.log(`Assigning the version "${safe_1.default.cyan(explicitlyPreferredVersion)}" for "${packageName}" ` +

...
class PublishUtilities {
        ...
        do {
            ...
            allPackages.forEach((project) => {
                ...
                if (projectHasChanged) {
                    console.log(`${os_1.EOL}* APPLYING: update ${project.packageName} to version ${versionPolicyChange.newVersion}`);
                }
    ...
    static _getChangeTypeForSemverReleaseType(releaseType) {
        switch (releaseType) {
            case 'major':
                return ChangeManagement_1.ChangeType.major;
            case 'minor':
                return ChangeManagement_1.ChangeType.minor;
            case 'patch':
                return ChangeManagement_1.ChangeType.patch;
            case 'premajor':
            case 'preminor':
            case 'prepatch':
            case 'prerelease':
                return ChangeManagement_1.ChangeType.hotfix;
            default:
    ...
    static _writePackageChanges(change, allChanges, allPackages, rushConfiguration, shouldCommit, prereleaseToken, projectsToExclude) {
        ...
        if (shouldCommit) {
            node_core_library_1.JsonFile.save(packageJson, packagePath, { updateExistingFile: true });
        }
        return packageJson;
    }
    ...
    static _addChange(change, allChanges, allPackages, rushConfiguration, prereleaseToken, projectsToExclude) {
        ...
        if (skipVersionBump) {
            ...
            if (change.changeType === ChangeManagement_1.ChangeType.hotfix) {
                ...
                const shouldBump = change.newVersion === undefined && change.changeType >= ChangeManagement_1.ChangeType.hotfix;

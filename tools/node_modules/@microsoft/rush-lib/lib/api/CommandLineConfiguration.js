...
class CommandLineConfiguration {
    ...
    constructor(commandLineJson, options = {}) {
            ...
            for (const command of commandsJson) {
                ...
                switch (command.commandKind) {
                    case RushConstants_1.RushConstants.phasedCommandKind: {
                        ...
                        normalizedCommand = Object.assign(Object.assign({}, command), { isSynthetic: false, associatedParameters: new Set(), phases: commandPhases, watchPhases, alwaysWatch: false, alwaysInstall: undefined });
                        ...
                        if (watchOptions) {
                            ...
                            for (const phaseName of watchOptions.watchPhases) {
                                const phase = this.phases.get(phaseName);
                                if (!phase) {
                                    throw new Error(`In ${RushConstants_1.RushConstants.commandLineFilename}, in the "watchPhases" property of the ` +
                                        `"${normalizedCommand.name}" command, the phase "${phaseName}" does not exist.`);
                                }
        ...
        if (!options.doNotIncludeDefaultBuildCommands) {
            ...
            if (!this.commands.has(RushConstants_1.RushConstants.rebuildCommandName)) {
                ...
                const rebuildCommand = Object.assign(Object.assign({}, DEFAULT_REBUILD_COMMAND_JSON), { commandKind: RushConstants_1.RushConstants.phasedCommandKind, isSynthetic: true, phases: buildCommandPhases, disableBuildCache: DEFAULT_REBUILD_COMMAND_JSON.disableBuildCache, associatedParameters: buildCommand.associatedParameters, watchPhases: new Set(), alwaysWatch: false, alwaysInstall: undefined });
        ...
        for (const dependency of phase.dependencies.self) {
            if (phasesInPath.has(dependency)) {
                throw new Error(`In ${RushConstants_1.RushConstants.commandLineFilename}, there exists a cycle within the ` +
                    `set of ${dependency.name} dependencies: ${Array.from(phasesInPath, (phase) => phase.name).join(', ')}`);
            }
            else {
    ...
    static tryLoadFromFile(jsonFilePath) {
        let commandLineJson;
        try {
            commandLineJson = node_core_library_1.JsonFile.loadAndValidate(jsonFilePath, CommandLineConfiguration._jsonSchema);
        }
        catch (e) {
    ...
    static loadFromFileOrDefault(jsonFilePath) {
        ...
        if (jsonFilePath) {
            try {
                commandLineJson = node_core_library_1.JsonFile.load(jsonFilePath);
            }
            catch (e) {
                if (!node_core_library_1.FileSystem.isNotExistError(e)) {
                    throw e;
    ...
    _translateBulkCommandToPhasedCommand(command) {
        ...
        const translatedCommand = Object.assign(Object.assign({}, command), { commandKind: 'phased', isSynthetic: true, associatedParameters: new Set(), phases, 
